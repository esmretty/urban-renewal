...
function rowHTML(p) {
  ...
  >
    ...
    <div class="c c-addr" title="${esc(roadOnly)}" data-label="地址"><span class="addr-text">${esc(roadOnly)}${p.address_inferred ? '<span class="inferred-tag">推測</span>' : ''}${p.is_foreclosure ? '<span class="fc-badge" title="法拍屋">法拍屋</span>' : ''}${p.is_remote_area ? `<span class="fc-badge" style="background:#9aa0a6" title="依政府河道/天險範圍判定為偏遠位置">偏遠路段</span>` : ''}${p.unsuitable_for_renewal ? `<span class="fc-badge" style="background:#9aa0a6" title="${esc(p.unsuitable_reason || '土地分區非住、商、工用地')}">特殊土地分區</span>` : ''}<a href="https://www.google.com/maps/search/${encodeURIComponent(fullAddress(p))}" target="_blank" rel="noopener noreferrer" class="map-link" onclick="event.stopPropagation()" title="Google Maps">📍</a></span>${(evBadge || archivedBadge || newBadge) ? `<span class="addr-badges">${evBadge}${archivedBadge}${newBadge}</span>` : ''}</div>
